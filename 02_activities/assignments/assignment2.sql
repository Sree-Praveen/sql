...
SELECT 
COALESCE(product_name, '') || ', ' || COALESCE(product_size, '') || ' (' || COALESCE(product_qty_type, 'unit') || ')' as new_column
FROM product;
...
SELECT 
    ...
    product_name,
    SUM(Revenue)
FROM (Select Distinct vendor_name, product_name, original_price*5 as Revenue 
